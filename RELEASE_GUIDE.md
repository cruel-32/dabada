...
7. **Guideline 5.2.3 - 지적 재산권**  
   Apple이 "타사 소스의 음악·동영상을 권한 없이 저장·다운로드하는 앱"이라 거절한 경우, 이는 **기능 자체의 규정 준수** 문제입니다.
   - **약관·면책·동의로 해결 불가**: Apple이 요구하는 것은 "원본 소스(YouTube, Instagram 등)의 **명시적 허가(authorization)**"입니다. "사용자가 권한 있는 콘텐츠만 저장하겠다고 동의한다"는 이용약관이나 면책 조항은 이를 대체하지 못합니다.
   - **가능한 대응**: (1) YouTube/Instagram 등 권리자로부터 라이선스·이용 허가 취득, (2) 본인 소유 콘텐츠만 저장 가능하도록 기능 제한, (3) 해당 기능 제거 또는 서비스 전환, (4) 법률·라이선스 전문가 상담
...
1. **1.0 준비 제출** (또는 해당 버전) 화면에서 **아래 체크리스트**대로 필수 항목을 모두 채운 뒤
2. **심사를 위해 제출** 버튼 클릭
...
#### 심사 제출 전 체크리스트 (에러 해결용)

"심사에 추가할 수 없음" / "이 페이지에 하나 이상의 오류가 있습니다" 가 나오면 아래를 **모두** 채우세요.

| 에러 메시지 | 해결 방법 (App Store Connect 위치) |
|-------------|-------------------------------------|
| **콘텐츠 권한 정보** / **개인정보 처리지침** | **앱 정보** → **개인정보** (또는 **앱 개인정보**): "개인정보 처리방침 URL" 입력. 예: `https://dabada.cloudish.cloud/ko/privacy` |
| **빌드를 선택** | **1.0 준비 제출** (또는 해당 버전) → **빌드** 영역에서 **+** 클릭 후 업로드된 빌드 선택 |
| **연락처 정보 섹션** | **앱 정보** (또는 **1.0 준비 제출** 하단) → **연락처 정보**: 이메일·전화번호·URL 등 필수 항목 입력 |
| **가격 등급** | **가격 및 배포** → **가격**: 무료면 **0원** 또는 **Free** 등급 선택, 유료면 해당 가격 등급 선택 |
| **한국어 - 지원 URL** (Guideline 1.5) | **지원 URL**에 **고객 지원 전용 페이지**를 입력. 홈(`/ko`)이 아닌 지원 페이지 필수. **권장**: `https://dabada.cloudish.cloud/ko/support` |
| **한국어 - 키워드** | **1.0 준비 제출** → **한국어** → **키워드**: 쉼표로 구분, 최대 100자 (예: `비디오,영상,다바다`) |
| **한국어 - 설명** | **1.0 준비 제출** → **한국어** → **설명**: 앱 설명 텍스트 (최대 4000자) |
| **앱 이름/부제목** (Guideline 2.3.7) | **앱 정보** 또는 **버전 정보** → **이름**, **부제목**: "download", "downloader" 포함 금지. 예: "Dabada Video" (O), "Dabada Video Downloader" (X) |

**한국어 메타데이터 입력 위치 요약**

- App Store Connect → **내 앱** → 해당 앱 선택 → **1.0 준비 제출** (또는 버전별 "준비 제출")
- 왼쪽 또는 본문에서 **앱 정보** / **버전 정보** 선택 후 **한국어** 탭(또는 언어 목록에서 한국어) 클릭
- **지원 URL**, **키워드**, **설명** 필드에 위 표대로 입력 후 저장

**개인정보 처리방침 URL**

- 이 프로젝트의 웹 앱에 개인정보 처리방침 페이지가 있으면 그 URL을 사용 (예: `https://dabada.cloudish.cloud/ko/privacy`).
- **앱 정보** → **개인정보** 섹션에서 "이 앱이 수집하는 데이터" 등도 필요한 경우 설정.
